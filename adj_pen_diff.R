...
fun.pen_setup <- function(data) {
  ...
  pen_source <- pen_add %>% 
    ...
    select(game_id, event_index, season, game_date, event_type, event_description, event_detail,
           event_team, event_player_1, event_player_2, home_team, away_team, home_lead, home_lead_state, game_period,  
           game_seconds, last_pen_time, game_strength_state, next_st_state1
    ) %>% 
    mutate(last_pen_time = ifelse(last_pen_time == 0, lag(last_pen_time), last_pen_time), 
           ...
           no_impact = ifelse(#(grepl("too many men", tolower(event_description)) == TRUE) == 1 | 
             (grepl("match", tolower(event_description)) == TRUE) == 1 | 
               #(grepl("officials - bench", tolower(event_description)) == TRUE) == 1 | 
               (game_strength_state %in% c("5v5", "4v4", "3v3") & next_st_state15 %in% c("5v5", "4v4", "3v3")) | 
               (game_strength_state == next_st_state15 & 
                  (game_strength_state != "5v3" & game_strength_state != "3v5" & game_strength_state != "Ev3" & game_strength_state != "3vE")) |
               pen_shot == 1 | 
               major == 1 | 
               next_st_state15 == 0, 
             1, 0)
    ) %>% 
    select(-c(next_st_state1, next_st_state2:next_st_state14)) %>% 
    select(game_id:game_strength_state, next_st_state15, pen_shot, major, double, team_pen, no_impact, cluster) %>%  
...
fun.pen_assign <- function(data) {
  ...
  # Initial gather
  test1 <- data %>% filter(no_impact == 0, cluster == 1) %>% 
  ...
  test1a <- test1 %>% select(-c(game_id, event_index, event_player_2)) %>% 
    gather(Column, Value, -pen_id, na.rm = TRUE) %>%
    count(pen_id, Value)  %>% 
    rename(take = Value)
  
  test1b <- test1 %>% select(-c(game_id, event_index, event_player_1)) %>% 
    gather(Column, Value, -pen_id, na.rm = TRUE) %>%
    count(pen_id, Value)  %>% 
    rename(draw = Value)
  ...
  test2a <- test1c %>% select(pen_id, take, n.x) %>% group_by(pen_id, take) %>% 
    summarise(take_n = as.numeric(sum(n.x))) %>% 
    rename(player = take)
  
  test2b <- test1c %>% select(pen_id, draw, n.y) %>% group_by(pen_id, draw) %>% 
    summarise(draw_n = as.numeric(sum(n.y))) %>% 
    rename(player = draw)
  
  test2c <- test2a %>% full_join(., test2b, by = c("pen_id", "player")) %>% 
    filter(!is.na(player)) %>% 
    ...
    mutate(offset1 = ifelse(pen_id == lag(pen_id) & 
                              take_n == lag(take_n) & 
                              draw_n == lag(draw_n) & 
                              diff == 0 & 
                              lag(diff) == 0, 1, 0), 
  ...
  test2d <- test2c %>% filter(offset == 1) %>% select(pen_id, offset) %>% group_by(pen_id) %>% 
    summarise(offset = first(offset))  
  
  # Assign
  test3 <- test2c %>% filter(offset != 1) %>% select(-c(offset))
  ...
  # Join Data
  pen_enhanced <- data %>% left_join(. , test4, by = c("game_id", "event_index")) %>% 
...
fun.pen_value_main <- function(pen_data, pbp_data) {
  ...
  pen_calc_main <- pen_data %>% filter(no_impact != 1) %>% 
    ...
    mutate(time_diff = game_seconds - last_pen_time, 
           ...
           EV_4v4_4v3 = ifelse(game_strength_state %in% c("4v4", "Ev4", "4vE") & next_st_state %in% c("4v3", "3v4"),
                               ifelse(game_period == 4, 
                                      ...
                                      ((penrate_GF[2,7] - penrate_GA[2,7])*
                                         (120 - (ifelse(game_seconds - last_pen_time > 120 | game_seconds - last_pen_time < 0, 60, game_seconds - last_pen_time)))/3600) + 
                                        (((penrate_GF[1,5] - penrate_GA[1,5])*
                                            (ifelse(game_seconds - last_pen_time > 120 | game_seconds - last_pen_time < 0, 60, game_seconds - last_pen_time)))/3600)), 
                               0), 
           
           EV_3v3_4v3 = ifelse(game_strength_state %in% c("3v3", "Ev3", "3vE") & next_st_state %in% c("3v4", "4v3"), # & game_period == 4, 
                               ((penrate_GF[3,7] - penrate_GA[3,7])*(ifelse(3900 - game_seconds > 120, 120, (3900 - game_seconds))))/3600, 
                               0), 
           ...
           PP_5v4_5v3_h = ifelse(game_strength_state %in% c("5v4", "Ev4") & next_st_state %in% c("5v3", "Ev3"),
                                 ifelse(game_seconds - last_pen_time < 120, 
                                        ((penrate_GF[4,6] - penrate_GA[4,6])*(120 - (game_seconds - last_pen_time)))/3600 + 
                                          ((penrate_GF[1,5] - penrate_GA[1,5])*(game_seconds - last_pen_time))/3600, 
                                        ((penrate_GF[1,5] - penrate_GA[1,5])*120)/3600), 
                                 0), 
           ...
           PP_5v4_4v4_h = ifelse((game_strength_state %in% c("5v4", "Ev4") & next_st_state =="4v4") | 
                                 ...
                                 ((penrate_GA[4,3] - penrate_GF[4,3])*(120 - (game_seconds - last_pen_time)))/3600 + 
                                   ((penrate_GF[1,5] - penrate_GA[1,5])*(game_seconds - last_pen_time))/3600, 
                                 0), 
           ...
           PP_5v3_4v3_h = ifelse((game_strength_state == "5v3" & next_st_state == "4v3"), 
                                 ((penrate_GA[5,7] - penrate_GF[5,7])*(120 - (game_seconds - last_pen_time)))/3600 + 
                                   ((penrate_GA[4,3] - penrate_GF[4,3])*(game_seconds - last_pen_time))/3600, 
                                 0), 
           ...
           PP_4v3_3v3_h = ifelse(game_strength_state %in% c("4v3") & next_st_state == "3v3", 
                                 ((penrate_GA[6,4] - penrate_GF[6,4])*(120 - (game_seconds - last_pen_time)))/3600 + 
                                   ((penrate_GF[3,7] - penrate_GA[3,7])*(game_seconds - last_pen_time))/3600, 
                                 0), 
           ...
           # Sum
           sub_sum = EV_5v5_5v4   + EV_4v4_4v3   + EV_3v3_4v3 + EV_5v5_5v3 + 
  ...
  pen_calc_verify <- pen_data %>% 
    ...
    mutate(event_player_1 = ifelse(event_player_1 == 0, NA, event_player_1), 
           ...
           verified1 = ifelse(first_assign == 1 & lead(game_seconds) == game_seconds & 
                                (lead(event_player_2, 2) == event_player_1 & lead(event_player_1, 2) == event_player_2), 1, 0), 
           verified1 = ifelse(is.na(verified1), 0, verified1), 
           verified2 = ifelse(first_assign == 1 & lead(game_seconds) == game_seconds & 
                                (lead(event_player_2, 1) == event_player_1 & lead(event_player_1, 1) == event_player_2), 1, 0), 
           verified2 = ifelse(is.na(verified2), 0, verified2), 
           verified3 = ifelse(first_assign == 1 & lead(game_seconds) == game_seconds & 
                                (lead(event_player_2, 1) == lead(event_player_1, 2) & lead(event_player_1, 1) == lead(event_player_2, 2)), 1, 0), 
  ...
  pen_calc_assign <- pen_calc_main %>% 
    filter(first_assign == 1) %>% select(game_id, event_index, take_assign, draw_assign)
...
fun.pen_value_xtra <- function(data) {
  ...
  pen_calc_xtras <- data %>% filter(pen_shot == 1 | major == 1) %>% 
    ...
    mutate(
      ...
      PS_EV_draw = ifelse(pen_shot == 1 & game_strength_state %in% c("5v5", "4v4", "3v3", "Ev5", "5vE"), .3194, 0), 
      PS_PP_draw = ifelse(pen_shot == 1 & game_strength_state %in% c("5v4", "4v5", "Ev4", "4vE", "5v3", "3v5", "4v3", "3v4"), .3194, 0), 
      ...
      maj_PP_take_h = ifelse(major == 1 & (game_strength_state %in% c("5v4", "Ev4") & next_st_state %in% c("5v3", "Ev3")),
                             ifelse(game_seconds - last_pen_time < 120, 
                                    ((penrate_GF[4,6] - penrate_GA[4,6])*(120 - (game_seconds - last_pen_time)))/3600 + 
                                      ((penrate_GF[1,5] - penrate_GA[1,5])*(300 - (game_seconds - last_pen_time)))/3600, 
                                    ((penrate_GF[1,5] - penrate_GA[1,5])*300)/3600), 0), 
      ...
      # Account for weird Majors
      #maj_PP_take_h = ifelse(major == 1 & maj_PP_take_h == 0 & maj_EV_take == 0 & maj_EV_draw == 0, 
      #                        ((penrate_GF[1,5] - penrate_GA[1,5])*120)/3600, maj_PP_take_h), 
      
      #maj_PP_take_a = ifelse(major == 1 & maj_PP_take_a == 0 & maj_EV_take == 0 & maj_EV_draw == 0, 
      #                       ((penrate_GF[1,5] - penrate_GA[1,5])*120)/3600, maj_PP_take_a), 
